...
export PYTHONPATH="/root/.cache/huggingface/modules:${PYTHONPATH:-}"
export VLLM_ALLOW_LONG_MAX_MODEL_LEN=1

project_name=${PROJECT_NAME:-"GRPO"}
exp_name=${EXP_NAME:-"deepmath-qwen14b-mainline-sync-grpo"}
...
# Older Qwen-14B is materially heavier than Qwen3-8B, so start from a
# conservative local-4GPU configuration and then scale up only after it boots.
