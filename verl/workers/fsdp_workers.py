...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def update_actor_stream(self, data: DataProto, zero_grad: bool = False, step_optimizer: bool = False):
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during update_actor_stream", logger=logger)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
            log_gpu_memory_usage("After offload actor optimizer during update_actor_stream", logger=logger)
